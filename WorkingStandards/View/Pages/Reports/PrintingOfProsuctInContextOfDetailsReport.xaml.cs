...
{
    /// <summary>
    /// Предпросмотр и печать отчёта [Печать по изделиям в разрезе деталей]
    /// </summary>
    /// <inheritdoc cref="Page" />
    public partial class PrintingOfProsuctInContextOfDetailsReport : IPageable
    {
        private const string ReportFileName = "PrintingOfProsuctInContextOfDetails.rdlc";
        ...
        public PrintingOfProsuctInContextOfDetailsReport()
        {
            InitializeComponent();
        ...
        public void AdditionalInitializeComponent()
        {
            ...
            const bool isProductSpecifiedOrAll = true;
            ...
            var product = parametersWindow.SelectedProductOrAll();
            ...
            _reportParameters = new[] { new ReportParameter("Date", loadDateTime.ToShortDateString()),
                new ReportParameter("WorkGuild",
                    workGuild == null ? "по всему заводу" : "цех " + workGuild.Id.ToString(CultureInfo.InvariantCulture))
            };
            try
            {
                var resultReportList = PrintingOfProsuctInContextOfDetailsService.GetPrintingOfProsuctInContextOfDetails(product, workGuild);
                const string dataSourceName = "PrintingOfProductInContextOfDetails";
